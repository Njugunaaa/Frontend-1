      ...
      <div className="grid gap-4">
        {events.map(event => (
          <div key={event.id} className="bg-white p-6 rounded-lg shadow-md flex justify-between items-start">
            <div>
              <h3 className="text-xl font-bold" style={{ color: '#7A030D' }}>{event.title}</h3>
              <p className="text-gray-600 mb-2">{event.description}</p>
              <p className="text-sm text-gray-500">
                Date: {event.date} {event.time && `at ${event.time}`}<br/>
              ...
              {event.image_path && (
                <img
                  ...
                  alt={event.title}
                  className="mt-2 w-full max-w-[300px] h-[400px] object-contain rounded shadow"
                />
              )}
            </div>
          ...
          <div className="bg-white p-8 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-4 pt-4">
                <button type="submit" className="px-6 py-2 rounded font-semibold text-white" style={{ backgroundColor: '#EB3237' }}>
